...
int main(int argc, char *argv[]) {
	...
	if (argc == 3 && (*argv[2] == 'e' || *argv[2] == 'd')) {
		...
		if (mode == 'e') {
			...
			printf("Error: File could not be encoded");
			exit(5);
		}
 ...
 * The decode operates much like the encode function. It has to
...
void decode(FILE *in, FILE *out) {
	...
	while ((newCode = (unsigned int) read12(in)) != EOF) {
		...
		/*
		 * This section of code checks for a case which
		 * would generate undefined code. It will handle
		 * it by decoding the last code and adding a single
	...
	/* Flush file and then close them */
	fflush(out);
	fclose(in);
	fclose(out);
	exit(0);
...
/*
 * This function is a hashing technique that finds the
 * table location for string/character combinations.
 * This function also handles collisions by jumping
 * around the table. If it walks through the table and
 * can't find an open spot it will reset the dictionary.
 */
...
/*
 * This function resets the dictionary once it is full.
 * If the program can't find an open space in the dictionary
 * it will reset the dictionary and then search again for the
 * next open spot.
 */
void resetDictionary() {
	int i;
